...
| --- | --- | --- |
| A01 Broken Access Control | Staff routes require auth, approved staff email domain, active user, confirmed staff MFA, and ETC staff title middleware. System administrator, ETC issuer, and executive roles are the only staff roles; only the ETC issuer can approve and issue certificates. | UAT role matrix signed off; negative access tests passing; production staff role review completed; `SECURITY_STAFF_MFA_REQUIRED=true`; `SECURITY_STAFF_EMAIL_DOMAINS` reviewed. |
...
| A06 Insecure Design | ETC approval, issuance, and printing require confirmed WanGov/GovPay payment. Approval and issuance are one action performed by the ETC issuer. System administrators create only ETC issuer and executive users. | Business process sign-off; abuse-case review for payment, issuance, printing, and verification flows. |
...
| A08 Software or Data Integrity Failures | Webhooks require an explicit shared secret, optional IP allowlist, payload size limit, replay/idempotency handling, and stored payload SHA-256/source/request metadata. WanGov production endpoints must use HTTPS, and checkout redirects are restricted to configured hosts. PDF/QR verification uses server records. | WanGov production callback contract verified; `WANGOV_WEBHOOK_SECRET` set to a distinct high-entropy value; `WANGOV_CHECKOUT_ALLOWED_HOSTS` reviewed; payment provider settlement records compared against ETC invoices and payments. |
| A09 Security Logging and Alerting Failures | Audit log service exists; session integrity failures, webhook rejections, certificate issuance, certificate printing, and public certificate verification attempts are logged. | Central log shipping/SIEM configured; alert thresholds for brute force, webhook rejection, invalid certificate verification bursts, and privilege changes. |
...
- Public ETC application, payment callbacks, and certificate verification remain internet-facing with throttles and token/code controls.
- Staff, HQ, admin, and certificate document routes require authenticated users from approved staff email domains, active accounts, staff membership, confirmed MFA, and ETC title-based authorization.
