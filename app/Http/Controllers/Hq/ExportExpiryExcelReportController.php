<?php
// FILE: app/Http/Controllers/Hq/ExportExpiryExcelReportController.php
...
class ExportExpiryExcelReportController extends Controller
{
    public function __invoke(Request $request): StreamedResponse
    {
        ...
        return response()->streamDownload(function () use ($query) {
            ...
            echo '<Worksheet ss:Name="HQ Expiry Report"><Table>';
    ...
    protected function buildExport(
        ...
        if ($scope === 'expired_active') {
            ...
            return ['hq-expired-active-permits-' . now()->format('Ymd-His') . '.xls', $query];
        }
        ...
        if ($startDate !== null && $endDate !== null) {
            ...
            return ['hq-expiring-range-permits-' . now()->format('Ymd-His') . '.xls', $query];
        }

        $query->whereDate('valid_until', '>=', today())
            ->whereDate('valid_until', '<=', today()->copy()->addDays($days));

        return ['hq-expiring-soon-permits-' . now()->format('Ymd-His') . '.xls', $query];
    }
