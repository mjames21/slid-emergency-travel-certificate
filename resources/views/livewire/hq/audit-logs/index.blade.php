<div class="rounded-xl bg-white p-5 shadow">
    <h1 class="text-2xl font-bold text-gray-900">Audit Logs</h1>

    <div class="mt-6 overflow-x-auto">
        <table class="min-w-full text-sm">
            <thead class="border-b text-left text-gray-500">
                <tr>
                    <th class="py-3 pr-4">Time</th>
                    <th class="py-3 pr-4">User</th>
                    <th class="py-3 pr-4">Airport</th>
                    <th class="py-3 pr-4">Desk</th>
                    <th class="py-3 pr-4">Action</th>
                    <th class="py-3 pr-4">Description</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($logs as $log)
                    <tr class="border-b">
                        <td class="py-3 pr-4">{{ optional($log->created_at)->format('Y-m-d H:i:s') }}</td>
                        <td class="py-3 pr-4">{{ $log->user?->name ?? 'System' }}</td>
                        <td class="py-3 pr-4">{{ $log->airport?->code ?? '—' }}</td>
                        <td class="py-3 pr-4">{{ $log->desk?->code ?? '—' }}</td>
                        <td class="py-3 pr-4">{{ $log->action }}</td>
                        <td class="py-3 pr-4">{{ $log->description }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="py-4 text-gray-500">No logs found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    </div>

    <div class="mt-6">
        {{ $logs->links() }}
    </div>
</div>
