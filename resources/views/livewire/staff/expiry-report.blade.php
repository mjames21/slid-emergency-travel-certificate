{{-- FILE: resources/views/livewire/staff/expiry-report.blade.php --}}
<div class="space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Staff Expiry Report</h1>
            <p class="text-sm text-gray-600">
                @if ($dashboardAirportName)
                    Expiry monitoring for {{ $dashboardAirportName }}.
                @else
                    Expiry monitoring.
                @endif
            </p>
        ...
        <div class="flex flex-wrap gap-3">
            <form method="GET" action="{{ route('staff.reports.permit-expiry.csv') }}">
            ...
            <form method="GET" action="{{ route('staff.reports.permit-expiry.xls') }}">
                <input type="hidden" name="scope" value="{{ $scope }}">
                <input type="hidden" name="start_date" value="{{ $start_date }}">
                <input type="hidden" name="end_date" value="{{ $end_date }}">
                <input type="hidden" name="search" value="{{ $search }}">
            ...
            <a href="{{ route('staff.reports.permit-expiry.csv', ['scope' => 'expiring_soon', 'days' => 7]) }}"
               class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700">
                Quick Expiring CSV
            </a>

            <a href="{{ route('staff.reports.permit-expiry.xls', ['scope' => 'expiring_soon', 'days' => 7]) }}"
               class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700">
                Quick Expiring Excel
            </a>

            <a href="{{ route('staff.reports.permit-expiry.csv', ['scope' => 'expired_active']) }}"
               class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700">
                Quick Expired CSV
            </a>

            <a href="{{ route('staff.reports.permit-expiry.xls', ['scope' => 'expired_active']) }}"
               class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700">
                Quick Expired Excel
            </a>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead>
                    <tr class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                        <th class="px-3 py-3">
                            <button type="button" wire:click="sortBy('permit_no')" class="font-semibold">Permit</button>
                        </th>
                        <th class="px-3 py-3">Traveler</th>
                        <th class="px-3 py-3">Passport</th>
                        <th class="px-3 py-3">Airport</th>
                        <th class="px-3 py-3">
                            <button type="button" wire:click="sortBy('valid_until')" class="font-semibold">Valid Until</button>
                        </th>
                ...
                <tbody class="divide-y divide-gray-200 bg-white text-sm text-gray-700">
                    @forelse ($permits as $permit)
                        ...
                        @endphp
                        <tr wire:key="staff-expiry-permit-{{ $permit->id }}">
                            ...
                            <td class="px-3 py-3">
                                <a href="{{ route('staff.permits.show', $permit) }}"
